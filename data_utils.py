...
class ASVDataset(Dataset):
    ...
    def __init__(self, database_path=None,protocols_path=None,transform=None, is_train=True, sample_size=None, 
    is_logical=True, feature_name=None, is_eval=False, eval_part=0):
        ...
        self.files_dir = os.path.join(self.data_root_dir, '{}_{}'.format(self.dset_name ,'wav'))
        print('files_dir',self.files_dir) 
        self.protocols_fname = os.path.join(self.protocols_dir, 'ASVspoof2019.{}.cm.{}.txt'.format(track, self.protocols_fname))
        print('protocols_fname',self.protocols_fname)
    ...
    def read_file(self, meta):
        sample_rate, data_x = wavfile.read(meta.path.replace("\\", "/"))
        data_y = meta.key   
    ...
    def _parse_line(self, line):  
        ...
        if self.is_eval:
            return ASVFile(speaker_id=tokens[0],
                file_name=tokens[1],
                path=os.path.join(self.files_dir, tokens[1] + '.wav'),
